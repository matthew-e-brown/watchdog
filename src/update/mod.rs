...
pub fn clone_and_push(gist_id: &str, new_ip: &str, use_ssh: bool) -> BoxResult<()> {
    ...
    data::write_md(&md_path, &updated_data)?;

    git::add_and_commit(path, format!("IP Address update - {}", updated_data.current.format_time()))?;
    git::push(path)?;
